...
> **Note**: These are complete end-to-end walkthroughs showing every step at once. When using the output-configurator skill interactively, you'll be guided through these steps **one at a time** with questions and confirmations at each stage. You won't see this entire document - instead, you'll only see the specific steps relevant to your chosen destination as we progress through the conversation.
