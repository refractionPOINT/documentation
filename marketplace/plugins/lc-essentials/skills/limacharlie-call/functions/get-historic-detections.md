...
- Detection data from LimaCharlie contains timestamps in **milliseconds** (e.g., `ts: 1764445185000`, `event_time: 1764445150453`)
...
```
Detection timestamp (ms): 1764445185000
                        ÷ 1000
API parameter (seconds):  1764445185
```
...
| Mistake | Symptom | Fix |
|---------|---------|-----|
| Using milliseconds instead of seconds | Empty results, count: 0 | Divide timestamp by 1000 |
| Using `ts` field from detection directly | Query returns nothing | `ts / 1000` for seconds |
| Using `event_time` from detection directly | Query returns nothing | `event_time / 1000` for seconds |
| Confusing with `get_detection` | Wrong API, different parameters | Use `get_detection(oid, detection_id)` to get ONE detection by ID |

**Example of the most common mistake:**

```
# WRONG - Using milliseconds from detection JSON
start: 1764445185000  # 13 digits = milliseconds = WRONG
end:   1764445300000

# CORRECT - Converted to seconds
start: 1764445185     # 10 digits = seconds = CORRECT
end:   1764445300
```
...
- **Timestamps in returned detection data**: Are in **milliseconds** (13 digits)
