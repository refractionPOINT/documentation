---
name: set-dr-general-rule
description: Create or update a custom Detection and Response (D&R) rule in the general namespace. Define detection logic, response actions, sensor targeting, and metadata. Use for building custom detections, implementing security policies, creating automated responses, or updating existing rules. Replaces rule if name exists.
