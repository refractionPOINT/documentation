---
name: set-lookup
description: Create or update a lookup table in LimaCharlie with enrichment data, threat intelligence, allowlists, blocklists, or reference data. Use this skill when users need to load threat feeds, create IP/domain/hash lists, store asset inventories, user mappings, or any key-value reference data for D&R rules and LCQL queries. Supports complex nested data structures. Updates existing tables or creates new ones.
