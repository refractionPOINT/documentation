---
name: set-extension-config
description: Create or update an extension configuration in a LimaCharlie organization. Use this skill when users need to configure, modify, or update extension settings. Creates a new configuration if it doesn't exist, or updates an existing one. Sets configuration data and marks it as enabled by default. Essential for managing extension behavior and settings.
