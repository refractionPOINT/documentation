---
name: get-event-types-with-schemas-for-platform
description: List event types with schemas filtered by platform (Windows, Linux, macOS, Chrome, etc.). Returns event types specific to the selected operating system or platform. Use when users need platform-specific telemetry, OS-targeted detection development, understanding platform capabilities, or filtering events by operating system.
