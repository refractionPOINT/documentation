...
- **name**: Description or name for the API key (required)
- **permissions**: List of specific permissions to grant (required)
  - Examples: ["sensor.get", "sensor.list", "sensor.task"]
  - Empty array = full organization access
...
3. List of specific permissions (recommended for security)
...
- Required parameters:
  ...
  - `permissions` (array): List of permissions (empty for full access)
