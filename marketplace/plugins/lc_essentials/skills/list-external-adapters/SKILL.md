---
name: list-external-adapters
description: List all external adapter configurations in a LimaCharlie organization. External adapters are integrations that process and ingest data from external sources like syslog servers, webhooks, custom APIs, third-party security tools, and other log sources. Use this skill when users need to view configured data ingestion adapters, audit external data source integrations, check which external systems are sending data, review adapter configurations and parsing rules, or troubleshoot external data ingestion. Shows adapter names, enabled status, tags, and configuration metadata.
