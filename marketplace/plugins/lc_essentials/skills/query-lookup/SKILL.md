---
name: query-lookup
description: Query a specific key in a lookup table to retrieve its value. Use this skill when users need to test lookup functionality, check if a specific IP/domain/hash exists in threat intelligence, verify allowlist entries, or test enrichment data before using in D&R rules. Returns the value for a key or indicates if the key is not found. Useful for validation and troubleshooting.
