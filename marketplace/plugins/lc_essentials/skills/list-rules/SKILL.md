---
name: list-rules
description: List all rules from a specific Hive in a LimaCharlie organization. Use this skill when users need to view, inventory, or audit rules from different Hives like D&R rules (dr-general, dr-managed), false positive rules (fp), or other custom Hive rule types. Generic Hive operation that works with any Hive name. Shows rule definitions, enabled status, tags, comments, and metadata. Essential for managing detection, response, and filtering rules.
