---
name: set-rule
description: Create or update a rule in a Hive in a LimaCharlie organization. Use this skill when users need to configure, modify, or update rules in any Hive like D&R rules (dr-general), false positive rules (fp), or other custom Hive types. Generic Hive operation that works with any Hive name. Creates new rule if it doesn't exist, or updates existing one. Essential for managing detection, response, and filtering rules.
