---
name: list-installation-keys
description: List and view all installation keys (sensor deployment credentials) in a LimaCharlie organization. Installation keys are used to deploy sensors and automatically tag them. Use this skill when users need to view existing installation keys, check which keys are available, audit sensor deployment configurations, or inspect key properties including tags, descriptions, creation dates, and IIDs. Common scenarios include sensor deployment planning, key management, security audits, and troubleshooting sensor enrollment.
