---
name: get-process-strings
description: Extract readable strings from process memory for malware analysis, credential hunting, configuration discovery, and identifying C2 servers. Returns ASCII and Unicode strings found in process memory. Use for memory forensics, malware investigation, IOC extraction, and understanding malicious process behavior.
allowed-tools: mcp__limacharlie__get_process_strings, Read
---
...
```
mcp__limacharlie__get_process_strings(
  oid="[organization-id]",
  sid="[sensor-id]",
  pid=[process-id]
)
...
For the MCP tool, this uses the dedicated `get_process_strings` tool.
