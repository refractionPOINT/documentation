---
name: add-output
description: Create a new output configuration in LimaCharlie to export data to external systems. Use this skill when users need to set up data routing, configure syslog forwarding, create S3 archives, set up webhooks, Slack notifications, or any data export destination. Supports all output modules including syslog, s3, gcs, webhook, slack, elastic, kafka, splunk, and more. Configure filtering by tags, sensors, event types, and detection categories.
