---
name: get-event-types-with-schemas
description: List all available event types with schemas in the LimaCharlie organization. Returns complete list of event type names that have schema definitions. Use when users need to discover available event types, browse telemetry capabilities, understand data coverage, or find specific event types before querying their schemas.
