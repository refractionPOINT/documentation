---
name: create-api-key
description: Create a new API key for a LimaCharlie organization with optional permission restrictions. Use this skill when you need to generate API credentials for integrations, automation, or third-party access. Supports creating full-access keys or restricted keys with specific permissions. The actual key value is only shown once during creation.
