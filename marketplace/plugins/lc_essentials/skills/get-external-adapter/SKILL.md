---
name: get-external-adapter
description: Get detailed configuration for a specific external adapter in a LimaCharlie organization. External adapters are integrations that process and ingest data from external sources like syslog servers, webhooks, custom APIs, and third-party security tools. Use this skill when users need to view a specific adapter's complete configuration, inspect parsing rules and field mappings, troubleshoot external data ingestion issues, review adapter connection details and credentials, or examine adapter metadata including creation info and error status. Requires the adapter name to retrieve.
