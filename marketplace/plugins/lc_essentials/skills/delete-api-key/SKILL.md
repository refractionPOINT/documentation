---
name: delete-api-key
description: Delete an API key from a LimaCharlie organization using its key hash. Use this skill when you need to revoke API access, remove compromised keys, clean up unused credentials, or implement key rotation. Requires the key hash which can be obtained from list-api-keys. This is a permanent operation that immediately revokes access.
