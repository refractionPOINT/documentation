---
name: delete-external-adapter
description: Delete an external adapter configuration from a LimaCharlie organization. External adapters are integrations that process and ingest data from external sources like syslog servers, webhooks, custom APIs, and third-party security tools. Use this skill when users need to remove external adapter integrations, decommission external data sources, clean up unused adapters, stop receiving data from specific external systems, or manage adapter lifecycle. Requires the adapter name to identify which adapter to delete. Permanent operation that stops data ingestion from the specified external source.
