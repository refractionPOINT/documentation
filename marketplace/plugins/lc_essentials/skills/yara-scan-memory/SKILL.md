---
name: yara-scan-memory
description: Scan process memory using YARA rules with process expression matching for targeted threat hunting. Takes YARA rule and process expression (e.g., name:chrome.exe) to scan multiple matching processes. Use for bulk process scanning, memory-based threat hunting, detecting fileless malware across process families, and comprehensive memory analysis.
allowed-tools: mcp__limacharlie__yara_scan_memory, Read
---
