---
name: get-dr-managed-rule
description: Get a specific managed Detection & Response rule by name from the managed namespace. Retrieve the full configuration including detection logic, response actions, and metadata for a single managed rule. Use when users need to view, inspect, analyze, or review a specific managed detection rule's configuration, understand detection logic, or troubleshoot rule behavior.
