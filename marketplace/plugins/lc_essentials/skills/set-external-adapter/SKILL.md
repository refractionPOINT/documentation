---
name: set-external-adapter
description: Create or update an external adapter configuration in a LimaCharlie organization. External adapters process and ingest data from external sources like syslog servers, webhooks, custom APIs, third-party security tools, firewalls, and other log sources. Use this skill when users need to configure new external data ingestion, update existing adapter settings, modify parsing rules and field mappings, configure connection parameters and credentials, enable or disable adapters, or change adapter tags and metadata. Requires adapter name and complete configuration object including parsing rules.
