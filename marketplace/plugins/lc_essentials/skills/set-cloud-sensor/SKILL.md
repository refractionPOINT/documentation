---
name: set-cloud-sensor
description: Create or update a cloud sensor configuration in a LimaCharlie organization. Cloud sensors collect telemetry from cloud platforms (AWS CloudTrail, Azure Activity Logs, GCP Audit Logs) and SaaS services (Office 365, Okta, GitHub) without requiring agent installation. Use this skill when users need to configure new cloud data sources, update existing cloud sensor settings, modify cloud integration credentials, enable or disable cloud sensors, or change cloud sensor tags and metadata. Requires sensor name and complete configuration object.
