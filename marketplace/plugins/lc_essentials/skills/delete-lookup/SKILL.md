---
name: delete-lookup
description: Delete a lookup table from LimaCharlie to remove enrichment data, threat intelligence, or reference data. Use this skill when users need to remove outdated threat feeds, delete test lookups, clean up unused reference data, or remove allowlists/blocklists. Deletion is immediate and permanent. D&R rules referencing the deleted lookup will fail.
...
mcp__limacharlie__lc_api_call(
  ...
  endpoint="api",
  method="DELETE",
  path="/hive/lookup/c7e8f940-1234-5678-abcd-1234567890ab/old-threat-ips"
)
```
...
mcp__limacharlie__lc_api_call(
  ...
  endpoint="api",
  method="DELETE",
  path="/hive/lookup/c7e8f940-1234-5678-abcd-1234567890ab/nonexistent-lookup"
)
```
