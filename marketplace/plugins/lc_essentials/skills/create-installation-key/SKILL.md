---
name: create-installation-key
description: Create a new installation key (sensor deployment credential) in a LimaCharlie organization. Installation keys are used to deploy and enroll sensors with automatic tagging. Use this skill when users need to generate new installation keys for sensor deployments, create keys with specific tags for organizational grouping, set up deployment credentials for different environments (production, development, test), or configure keys with descriptions. Supports creating keys with tag lists and optional quotas for controlled sensor enrollment.
