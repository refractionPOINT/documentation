...
description: Retrieve the list of platform names from LimaCharlie's ontology (Windows, Linux, macOS, Chrome, Android, etc.). Returns canonical platform identifiers used throughout the API. Use when users need valid platform names for filtering, sensor queries, event type filtering, or cross-platform detection development. Essential for platform-specific operations.
