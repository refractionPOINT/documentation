---
name: list-outputs
description: List and view all configured outputs in a LimaCharlie organization. Use this skill when users need to see output configurations, check data routing, view syslog destinations, S3 buckets, webhooks, or any data export settings. Retrieves output names, modules (syslog, s3, webhook, slack, etc.), types (event, detect, audit), and full configurations for troubleshooting, auditing, or understanding where data flows.
