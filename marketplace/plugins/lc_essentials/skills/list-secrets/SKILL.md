---
name: list-secrets
description: List all secret names stored in a LimaCharlie organization without revealing values. Use this skill when users need to see what secrets exist, audit stored credentials, check for secret availability, or verify secret names before referencing them in configurations. Returns only secret names, not actual values, for security. Secrets store API keys, passwords, tokens, and sensitive credentials used in outputs, integrations, and D&R rules.
