---
name: get-cloud-sensor
description: Get detailed configuration for a specific cloud sensor in a LimaCharlie organization. Cloud sensors are integrations that collect telemetry from cloud platforms and SaaS services without requiring agent installation. Use this skill when users need to view a specific cloud sensor's complete configuration, inspect cloud integration settings, troubleshoot cloud data source issues, review sensor credentials or connection details, or examine cloud sensor metadata including creation info and error status. Requires the sensor name to retrieve.
