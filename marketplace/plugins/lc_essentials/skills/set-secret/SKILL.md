---
name: set-secret
description: Store a secret securely in LimaCharlie for use in outputs, D&R rules, and integrations. Use this skill when users need to save API keys, passwords, tokens, access keys, or any sensitive credentials. Secrets are encrypted at rest and can be referenced in configurations using [secret:name] syntax. Creates new secrets or updates existing ones.
