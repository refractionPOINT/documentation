...
description: Retrieve detailed schema definition for a specific event type in LimaCharlie. Use when users need to understand event structure, available fields, data types, or element names for a particular event type like DNS_REQUEST, PROCESS_START, NETWORK_CONNECTIONS. Essential for query building, D&R rule creation, data analysis, and understanding telemetry structure.
...
mcp__limacharlie__lc_api_call(
  ...
  endpoint="api",
  method="GET",
  path="/orgs/[organization-id]/schema/[event-type-name]"
)
```
...
- Endpoint: `api`
- Method: `GET`
- Path: `/orgs/{oid}/schema/{name}` where `{name}` is URL-encoded
