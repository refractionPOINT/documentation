...
| The scan could not run | `neutral`, titled "Code scan could not complete". An outage on our side does not block your merge |

The check shows the full counts and up to 50 annotations, which is GitHub's
limit per update. Several pushes in quick succession produce one check, on the
newest commit.
...
A change made on GitHub can take up to a minute to show. Choose **Check again**
to refresh. A working webhook is re-checked every 5 minutes.
...
2. **GitHub delivered the event.** On GitHub, open the App's settings →
   **Advanced → Recent Deliveries**. Look for a `push` or `pull_request`
   delivery answered with `200`. A `401` means the secrets differ, so use
   [Re-sync webhook secret](#sync-webhook-secret). No delivery at all usually
   means the App is not installed on that repository.
...
A check that is declined leaves nothing on the pull request. The reasons are:

- `pr_checks` is off for that repository, or no enabled policy selects it;
- the App lacks **Checks** or **Pull requests: Read and write**
...
- the connection used its daily budget of 500 pull-request writes;
...
INSTALLATION_KEY=<an installation key for the organization>
