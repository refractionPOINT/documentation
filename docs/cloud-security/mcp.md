...
| `cloudsec_code_autofix` | Reserved. Refuses every call with a machine-readable reason |
...
Dependency fix pull requests are not available yet. Until they are, remediate from the finding
itself: `cloudsec_code_findings` returns `code.fixed_version` for a dependency vulnerability, and
`cloudsec_code_scan_local` confirms the bump before you push.
