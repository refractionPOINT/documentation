...
>
> The entries still shown in uppercase (`ACK_MESSAGES`, `BACKOFF`, `SYNC`,
> `UPDATE`, ...) are sensor-to-cloud protocol messages rather than
> `deployment` target events; they are documented here for reference and are
> not addressable from a D&R rule.
...
> **This event is emitted at most once per sensor per 24-48 hours.** The first
