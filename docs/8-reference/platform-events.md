...
> **Matching these in a D&R rule.** The names below that appear in lowercase
> (`deleted_sensor`, `enrollment`, `sensor_over_quota`, ...) are the literal
> `routing/event_type` values on the wire, and that is what a rule's `event:`
> or `events:` selector is compared against. That comparison is **exact and
> case-sensitive**, and `case sensitive: false` does *not* apply to it -- it
> only affects operators like `is` and `contains`. A rule written as
> `event: DELETED_SENSOR` therefore loads without error, reports no problem,
> and silently never matches anything. Copy the value out of the sample event.
>
> The entries still shown in uppercase are **not** `deployment` target events,
> and their names really are uppercase -- do not lowercase them. They are a
> mix: some are sensor-side events that reach the `edr` target under exactly
> that name (`DATA_DROPPED`), and some are sensor/cloud protocol messages that
> never surface to a rule at all (`BACKOFF` is a wire command). They are
> listed here because they show up in support conversations, not because they
> are all matchable.
...
> **This event is normally emitted only once per sensor per 24-48 hours.** The first
> time a deleted sensor is refused, the cloud also records the refusal so that
> subsequent connections from that sensor are dropped cheaply, without
> re-evaluating it -- which means no further `deleted_sensor` event is produced
> for the rest of that window. Restarting the sensor service does **not**
> regenerate the event.
>
> The practical consequence when pairing this with the
> [`undelete sensor`](response-actions.md#undelete-sensor) response action: the
> rule has to already be in place *before* the sensor is deleted. If you delete
> a sensor first and add the rule afterwards, the one event that would have
> triggered it has already gone by, and you will see nothing until the window
> lapses. To test, add the rule first, then delete a sensor and let it
> reconnect.
...
This platform event is generated by the LimaCharlie backend when a sensor disconnects from the cloud. This event applies to all sensor types (Windows, macOS, Linux, Chrome, Edge) and is generated server-side, not by the endpoint itself.
