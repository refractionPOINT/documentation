...
| `POST /actions/bulk/preview` | Preview a bulk remediation over a caller-supplied selection and mint the confirmation that authorizes exactly that set. Reads only; mints no job. Requires `mailsec.get`. See [Bulk Remediation](remediation.md) |
