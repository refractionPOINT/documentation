...
This output sends each event, detection, audit, deployment or artifact in a separate POST webhook.

- `dest_host`: the IP or DNS, port and page to HTTP(S) POST to, format `https://www.myorg.com:514/whatever`.
- `secret_key`: a shared secret of your choice. It computes an HMAC (SHA256) signature of the webhook to check authenticity. [See "Webhook Details" section.](../allowlisting.md)
