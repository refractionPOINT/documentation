...
class ClaudeClient:
    ...
    def run_subagent_prompt(
        ...
        # --output-format json wraps response in JSON with metadata
        result = subprocess.run(
        ...
        # Parse JSON wrapper to extract actual response
        try:
            wrapper = json.loads(result.stdout)
            actual_response = wrapper.get('result', '')
        except json.JSONDecodeError as e:
            raise RuntimeError(f"Failed to parse Claude JSON wrapper: {e}\nOutput: {result.stdout[:500]}")

        # Clean the response to extract JSON (handles markdown code blocks)
        return self._clean_json_response(actual_response)
