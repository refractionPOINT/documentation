...
**Option 1 - Logging In**
 The simplest is to login to an Organization using an [API key](../Access_and_Permissions/api-keys.md).
...
The `Payloads` can be used to manage various executable [payloads](../../Sensors/Endpoint_Agent/payloads.md) accessible to sensors.

#### Replay

The `Replay` object allows you to interact with [Replay](../../Detection_and_Response/replay.md) jobs managed by LimaCharlie. These allow you to re-run D&R Rules on historical data.
...
The `Webhook` object demonstrates handling [webhooks emitted by the LimaCharlie cloud](../../Sensors/Adapters/Adapter_Tutorials/tutorial-creating-a-webhook-adapter.md), including verifying the shared-secret signing of the webhooks.
...
[LimaCharlie Query Language (LCQL)](../../Query_Console/lcql-examples.md) provides a flexible, intuitive and interactive way to explore your data in LimaCharlie.
...
ARLs can be used in the [YARA manager](../../Add-Ons/Extensions/LimaCharlie_Extensions/ext-yara-manager.md) to import rules from GitHub repositories and other locations.
...
Shortcut utility to perform [Replay](../../Detection_and_Response/replay.md) jobs from the CLI.
