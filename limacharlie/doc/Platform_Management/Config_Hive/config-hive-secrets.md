...
The most common usage is for storing secret keys used by various [Adapters](../../Sensors/Adapters/adapter-usage.md) or [Outputs](../../Outputs/testing-outputs.md). By referencing `secrets` within the Config Hive, we can configure these services without needing to reveal secret keys to all users.
