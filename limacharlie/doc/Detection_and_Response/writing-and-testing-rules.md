...
Detection & Response () Rules are similar to Google Cloud Functions or AWS Lambda. They allow you to push D&R rules to the LimaCharlie cloud where the rules will be applied in real-time to data coming from the sensors.
...
For a full list of all rule operators and detailed documentation see the [Detection and Response](../Detection_and_Response/Reference/detection-logic-operators.md) section.
...
The goal of is code lab will be to create a D&R rule to detect the MITRE ATT&CK framework [Control Panel Items](https://attack.mitre.org/techniques/T1196/) execution.
...
Interacting with LC can always be done via the [web app](https://app.limacharlie.io) but day to day operations and automation can be done via the Command Line Interface (CLI). This will make following this code lab easier.
...
In this case, we only care about [CODE_IDENTITY](../Events/Endpoint_Agent_Events_Overview/reference-edr-events.md#codeidentity) events. We also know that our rule will use more than one criteria, and those criteria will be AND-ed together because we only want to match when they all match.
...
With this plan, #1 and #2 lend themselves well to [unit tests](https://en.wikipedia.org/wiki/Unit_testing) while #3 can be done more holistically by using Replay to run historical events through the rule and evaluate if there are any [false positives](https://en.wikipedia.org/wiki/False_positives_and_false_negatives).
...
Once your rule is done and you've evaluated various events for matches, you can move these to [D&R Rules Unit Tests](../Detection_and_Response/unit-tests.md) so that the tests are run during rule update.
