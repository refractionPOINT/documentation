...
Template strings can also be used with in conjunction the `client_options.mapping.transform` option in [Adapter configuration](../Sensors/Adapters/adapter-usage.md). These allow you to modify data prior to ingestion, having control over *what* fields get ingested and resulting field names.
