...
It is also possible to deploy a LimaCharlie [Adapter](../../Adapters/adapter-deployment.md) pointed to `auth.log` to collect and stream the events in directly. Note that Adapters will create a separate telemetry "stream" - thus, it is recommended to combine file types where possible.
...
More details on configuration files and adapter usage can be found [here](../../Adapters/adapter-usage.md).
