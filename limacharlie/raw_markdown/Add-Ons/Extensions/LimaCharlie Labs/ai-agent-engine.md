...
The AI Agent definition themselves are managed in the `ai_agent` [Hive](../../../Platform%20Management/Config%20Hive/config-hive-secrets.md) Configurations and can be managed across tenants using the Infrastructure as Code extension. This hive requires the `ai_agent.*` permissions.
...
This means agents can be invoked in a fully automated fashion based on events, detections, audit messages or any other [target](../../../Detection%20and%20Response/detection-on-alternate-targets.md) of rules. But it can also be used in an ad-hoc fashion triggered manually.
...
The credentials provided to the engine are simply a LimaCharlie API key, we recommend storing it in a [secret](../../../Platform%20Management/Config%20Hive/config-hive-secrets.md) and referencing as `hive://secret/my-lc-creds`.
...
The tools available to the AI Agents are the same ones available from the official [LimaCharlie MCP Server](../../../Connecting/mcp-server.md).
