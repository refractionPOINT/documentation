# Linux Log Collection Guide for LimaCharlie

This guide covers how to collect various Linux system logs into LimaCharlie using USP (Universal Sensor Protocol) adapters. LimaCharlie provides flexible log collection capabilities through file monitoring and syslog ingestion.

## Collection Methods

### 1. File Adapter (Recommended for Log Files)
...
file:
  client_options:
    ...
    mapping:
      parsing_grok: "%{SYSLOGTIMESTAMP:date} %{HOSTNAME:host} %{DATA:service}(?:\\[%{POSINT:pid}\\])?: %{GREEDYDATA:message}"
...
file:
  client_options:
    ...
    mapping:
      parsing_grok: "%{SYSLOGTIMESTAMP:date} %{HOSTNAME:host} kernel: %{GREEDYDATA:message}"
      sensor_hostname_path: "host"
...
file:
  client_options:
    ...
    mapping:
      parsing_grok: "type=%{DATA:audit_type} msg=audit\\(%{NUMBER:timestamp}:%{NUMBER:serial}\\): %{GREEDYDATA:audit_data}"
...
Journalctl is the modern logging solution for systemd-based Linux distributions. It can output in JSON format for structured parsing.
...
1. **Use JSON format when possible** - Modern logs often support JSON output, which provides better structure and parsing.

2. **Configure appropriate Grok patterns** - Grok provides pre-built patterns for common log formats and is easier to maintain than regex. Use `parsing_grok` over `parsing_re` when possible.

3. **Set sensor_seed_key appropriately** - Use descriptive names that identify the log source for easier management.

4. **Monitor file permissions** - Ensure the adapter has read access to log files.

5. **Consider log rotation** - The file adapter handles rotation automatically, but test with your specific setup.

6. **Use backfill carefully** - Only enable for initial historical data collection to avoid duplicates.

7. **Implement proper field mapping** - Extract hostname, timestamps, and event types for better searchability.

8. **Pattern testing** - Test Grok patterns against sample log lines before deployment. Common patterns include `%{COMMONAPACHELOG}`, `%{SYSLOGTIMESTAMP}`, and `%{NGINXACCESS}`.

## Security Considerations

- Ensure proper file permissions for log access
- Use TLS for syslog transmission when possible
- Implement proper authentication for USP connections
- Consider data sensitivity when choosing collection methods
- Regular monitoring of adapter health and connectivity
