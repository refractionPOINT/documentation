...
When you run Zeek, it processes the PCAP file using the following command:

```bash
/opt/zeek/bin/zeek -C LogAscii::use_json=T --no-checksums --readfile /path/to/your.pcap
```

Upon running Zeek, several JSON log files are generated. The log files are parsed and pushed into the `ext-zeek` sensor timeline, where they can be queried and used for detection and response.
