...
Creating a lookup enables you to create a list that you can use as part of detection and response rules. Once in place, you can refer to it using the `op: lookup` rule with a reference to your add-on looking like `resource: hive://lookup/my-lookup-name`.
...
3. As an Authenticated Resource Locator.
...
LimaCharlie understand the MISP format, regardless of how it is ingested. That being said, the classic way of ingesting it would be to ingest the MSIP Events use an ARL on a MISP REST API with one of the supported ARL authentication types like `basic`.

For example: `[https,misp.my.corp.com/events/1234,basic,myuser:mypassword]`.

### Reference D&R Rules

To put a Lookup "into effect", you need a detection and response rule. The Lookup is a list of elements while the rule describes what you want to look for in that list.
